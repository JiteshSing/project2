...
class PortfoliomanagementCtl(BaseCtl):
    ...
    def input_validation(self):
        ...
        if (DataValidator.isNull(self.form[" initialinvestmentAmount"])):
            ...
            if (DataValidator.is_0(self.form[' initialinvestmentAmount'])):
                inputError[
                    'initialinvestmentAmount'] = " initialinvestmentAmount can not be 0 or less than 0"
                self.form['error'] = True
    ...
    def save(self, request, params={}):
        ...
        if (self.input_validation()):
            ...
            if (self.form["id"] > 0):
                dup = Portfoliomanagement.objects.exclude(id=self.form['id']).filter(
                    portfolioName=self.form["portfolioName"])
                if (dup.count() > 0):
                    res["error"] = True
