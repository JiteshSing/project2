...
class StaffmemberCtl(BaseCtl):
    ...
    def preload(self, request, params={}):
        preloadList = [
            {'did': 1, 'division': "Web Developer"},
            {'did': 2, 'division': "Frontend"},
            {'did': 3, 'division': "Backend"},
    ...
    def input_validation(self):
        ...
        if (DataValidator.isNull(self.form['did'])):
            self.form["error"] = True
            inputError["did"] = "division can not be null"
        ...
        if DataValidator.isNull(self.form['joiningDate']):
            ...
            if DataValidator.isDate(self.form['joiningDate']):
                inputError[
                    'joiningDate'] = "Incorrect joiningDate format, should be DD-MM-YYYY format and joiningDate should in past or present"
                self.form['error'] = True
        ...
        elif (DataValidator.max_len_50(self.form['previousEmployer'])):

            inputError['previousEmployer'] = "previousEmployer can should be below 50 character"
    ...
    def input_validation1(self):
        ...
        if (DataValidator.isNotNull(self.form["previousEmployer"])):
            if (DataValidator.isalphacheck(self.form['previousEmployer'])):
                inputError['previousEmployer'] = "previousEmployer contain only letter"
                self.form['error'] = True
            else:
                if (DataValidator.max_len_50(self.form['previousEmployer'])):
                    inputError['previousEmployer'] = "previousEmployer can should be below 50 character"
                    self.form['error'] = True

        if (DataValidator.isNotNull(self.form['fullName'])):
            if (DataValidator.isalphacheck(self.form['fullName'])):
                inputError['fullName'] = "fullName contain only letter"
                self.form['error'] = True
            else:
        ...
        if DataValidator.isNotNull(self.form['joiningDate']):
            if DataValidator.isDate(self.form['joiningDate']):
                inputError[
                    'joiningDate'] = "Incorrect joiningDate format, should be DD-MM-YYYY format and joiningDate should not be in past "
                self.form['error'] = True
    ...
    def save(self, request, params={}):
        ...
        if (self.input_validation()):
            ...
            if (self.form["id"] > 0):
                ...
                if (dup.count() > 0):
                    res["error"] = True
                    res["message"] = "fullName  already exists"
                else:
                ...
                duplicate = Staffmember.objects.exclude(id=self.form["id"]).filter(fullName=self.form["fullName"])
