...
class ContactCtl(BaseCtl):
    ...
    def input_validation(self):
        ...
        if (DataValidator.isNull(self.form['cid'])):
            self.form["error"] = True
            inputError["cid"] = "city can not be null"
    ...
    def search(self, request, params={}):
        ...
        if (c != None):
            ...
            res["MaxId"] = c["MaxId"]
            res["index"] = c["index"]
            res["LastId"] = Contact.objects.last().id
            res["error"] = False
        else:
            res["error"] = True
            res["message"] = "No record found"
    ...
    def form_to_model(self, obj):
        ...
        print("ORS API Contact ============ Form to model------------------------")
        pk = int(self.form["id"])
        if (pk > 0):
            obj.id = pk
        obj.name = self.form["name"]
        obj.city = preload_list[index]["city"]
    ...
    def save(self, request, params={}):
        ...
        if (self.input_validation()):
            ...
            if (self.form["id"] > 0):
                dup = Contact.objects.exclude(id=self.form['id']).filter(mobile=self.form["mobile"])
                if (dup.count() > 0):
                    res["error"] = True
                ...
                duplicate = Contact.objects.filter(mobile=self.form["mobile"])
